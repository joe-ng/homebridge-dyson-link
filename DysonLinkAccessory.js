...
class DysonLinkAccessoryHelper {
    ...
    initFanState() {
        ...
        fan.getCharacteristic(Characteristic.On)
            .on("get", this.device.isFanOn.bind(device))
            .on("set", this.device.setFanOn.bind(device));
        fan.getCharacteristic(Characteristic.RotationSpeed)
            .on("get", this.device.isRotate.bind(device))
            .on("set", this.device.setRotate.bind(device));

        this.accessory.addService(Service.Switch , "Rotation - " + this.displayName, "Rotate")
            .getCharacteristic(Characteristic.On)
            .on("get", this.device.getRotateSpeed.bind(device))
            .on("set", this.device.setRotateSpeed.bind(device));

        this.accessory.addService(Service.Switch, "Auto - " + this.displayName, "Auto")
            .getCharacteristic(Characteristic.On)
            .on("get", this.device.isFanAuto.bind(device))
            .on("set", this.device.setFanAuto.bind(device));
